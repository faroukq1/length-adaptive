...
\begin{flushleft}
  ...
  {\footnotesize \textbf{Specialty:} Information Systems and Decision}
\end{flushleft}
...
This thesis addresses two open challenges in sequential recommendation systems: (1) the adaptive fusion of graph-based and sequence-based representations for users with heterogeneous interaction histories, and (2) the systematic evaluation of item textual description strategies for Large Language Model (LLM) embedding generation.




\textbf{First contribution -- Length-Adaptive Hybrid GNN-Transformer.} We propose a hybrid architecture that combines a Graph Neural Network (LightGCN) with a Transformer (BERT4Rec) through a user-specific fusion weight $\lambda(u)$, derived directly from the length of each user's interaction history. This mechanism gives more weight to global collaborative signals when the user history is short, and favours local sequential patterns when the history is long. Experiments on MovieLens-1M and ML-100K show that the proposed model \mbox{\mbox{HybridFixed}} outperforms the BERT4Rec baseline by +8.1\% in HR@10, +11.5\% in NDCG@10, and +14.2\% in MRR. On ML-100K, the improvements are dataset-dependent, with TCN-BERT4Rec emerging as the strongest variant. An ablation study confirms the independent contribution of both branches and the importance of the adaptive fusion design.

\textbf{Second contribution -- PromptCraft-SeqRec.} We design and evaluate seven distinct item description strategies (ranging from title-only to rich structured and user-centric prompts) for LLM-enhanced sequential recommendation. Using the LLM2Sequential pipeline with BGE-M3 embeddings and SASRec as the recommender, experiments on the Amazon Beauty dataset reveal that the structured attributes strategy (T2) improves NDCG@10 by +4.76\% over the title-only baseline, while rich prose (T3) shows no gain ($-0.61\%$), demonstrating that description format matters more than length. Intrinsic embedding analysis shows no direct correlation between average pairwise distance and recommendation performance.
...
\section*{Résumé}
\vspace{0.5cm}
Ce mémoire aborde deux défis ouverts dans les systèmes de recommandation séquentielle : (1) la fusion adaptative des représentations basées sur les graphes et les séquences pour des utilisateurs aux historiques d'interaction hétérogènes, et (2) l'évaluation systématique des stratégies de description textuelle des items pour la génération d'embeddings par grands modèles de langage (LLM).

\textbf{Première contribution -- Hybrid GNN-Transformer adaptatif à la longueur de l'historique.} Nous proposons une architecture hybride combinant un réseau de neurones graphiques (LightGCN) avec un Transformer (BERT4Rec) via un poids de fusion spécifique à l'utilisateur $\lambda(u)$, dérivé directement de la longueur de l'historique de chaque utilisateur. Ce mécanisme accorde plus de poids aux signaux collaboratifs globaux lorsque l'historique est court, et favorise les motifs séquentiels locaux lorsqu'il est long. Les expériences sur MovieLens-1M et ML-100K montrent que le modèle proposé (\mbox{HybridFixed}) dépasse la ligne de base BERT4Rec de +8.1\% en HR@10, +11.5\% en NDCG@10 et +14.2\% en MRR. Sur ML-100K, \mbox{TCN-BERT4Rec} donne les meilleurs résultats, la performance dépendant du jeu de données. Une étude d'ablation confirme la contribution indépendante des deux branches et l'importance de la fusion adaptative.

\textbf{Deuxième contribution -- PromptCraft-SeqRec.} Nous concevons et évaluons sept stratégies distinctes de description d'items (allant du simple titre à des descriptions structurées et centrées sur l'utilisateur) pour la recommandation séquentielle enrichie par LLM. En utilisant le pipeline LLM2Sequential avec les embeddings BGE-M3 et SASRec comme système de recommandation, les expériences sur le jeu de données Amazon Beauty révèlent que la stratégie des attributs structurés (T2) améliore le NDCG@10 de +4.76\% par rapport à la baseline du titre seul, tandis que les descriptions en langage naturel (T3) n'apportent aucun gain ($-0.61\%$), démontrant que le format de la description est plus important que sa longueur. L'analyse intrinsèque des embeddings ne montre aucune corrélation directe entre la distance cosinus moyenne et la performance de recommandation.
...
\textbf{Oussama Safwan Souici}, \textbf{Aymen Kennouche}, \textbf{Mezouari Abdelkader}, \textbf{Deneche Seif El Islam} and
\textbf{Nezzar Nour El Houda} -- your simple but powerful words, your sincere smiles,
...
\begin{center}
  \itshape\large
  First and foremost, all praise and thanks are due to \textbf{Allah}, the Most Gracious, the Most Merciful, for granting me the strength, patience, and guidance to complete this work. Alhamdulillah.\\[24pt]

  To my beloved parents,\\[8pt]
  for every sacrifice they made and for their unwavering support throughout my journey.\\[24pt]

  To my family and friends,\\[8pt]
  for their constant encouragement and support.\\[24pt]

  To my dear friends outside the university,\\[8pt]
  for your sincere friendship, your laughter, and your unwavering presence in my life. You are a true blessing.\\[24pt]

  To the entire SID cohort (2024--2026),\\[8pt]
  for the unforgettable journey, the shared efforts, and the wonderful memories. You made this experience truly unique.\\[24pt]
...
baseline on ML-1M. On ML-100K, TCN-BERT4Rec achieved the best performance, confirming that the most effective architecture depends on dataset scale and sequence length distribution. The per-bucket analysis confirmed that the adaptive fusion
