...
% FIX: rephrased the high-similarity "Netflix, Amazon, Spotify" sentence
...
\noindent\textit{Parts of this chapter have been accepted for publication as: Farouk Ouled Meriem, Samira Lagrini, Sarra Benzaid, and Nada Zeraoui, ``Dynamic Sequence Modeling via Hybrid GNN-Transformer Architecture for Robust Sequential Recommendation,'' Proceedings of the 2nd National Conference on Artificial Intelligence and Digital Transition (CAIDT'26), Ouargla, Algeria, April 29--30, 2026.}
...
\midrule
\multicolumn{3}{l}{*TCN-BERT4Rec and TGT-BERT4Rec use the same general hyperparameters (embedding dim 64, 2 heads, 2 blocks, dropout 0.2).} \\
\bottomrule
\end{tabular}
\end{table}
...
% FIX: renamed from \section{Results} to avoid duplicate label conflict with
%      the second results section below. Original had two \section{Results}
%      with the same \label{sec:results} which caused LaTeX cross-reference errors.
...
\begin{table}[h]
\centering
\caption{Test performance on MovieLens-1M. Best results bold, second-best underlined. \(\bullet\) = proposed hybrid.}
...
\begin{table}[h]
\centering
\caption{Test performance on MovieLens-100K. Best results bold, second-best underlined. \(\bullet\) = proposed hybrid.}
...
all metrics and model variants. Results are organized around three
complementary views: a full metric heatmap, a per-history-length
breakdown, and a generalization analysis through the val--test delta
curves.
...
\begin{figure}[H]
    ...
    \caption{Test metrics heatmap on ML-1M. Darker cells indicate higher
             values. Hybrid models are prefixed with BERT-Hybrid.}
...
\begin{figure}[H]
    ...
    \caption{Test metrics heatmap on ML-100K. Darker cells indicate higher
             values.}
...
\begin{figure}[H]
    ...
    \caption{NDCG@10 broken down by user history length group on ML-1M.
             Short: \(n_u \leq 10\); Medium: \(10 < n_u \leq 30\); Overall:
             all users.}
...
\begin{figure}[H]
    ...
    \caption{NDCG@10 broken down by user history length group on ML-100K.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.80\textwidth]{picture/figures/1m/delta_ndcg_overfitting_check_1m.png}
    \caption{\(\Delta\)NDCG@10 (Val \(-\) Test) per epoch on ML-1M for all four hybrid models.    The green band marks the healthy range (\(\pm 0.01\)). Red dots indicate epochs where the gap exceeds the threshold.}
    \label{fig:delta_1m}
\end{figure}

Figure~\ref{fig:delta_1m} plots the difference between validation and
test NDCG@10 at each epoch for all four hybrid models on ML-1M. A curve
oscillating around zero within the green band indicates that the model
generalizes consistently, with no systematic gap between the validation
and test sets. \textbf{BERT-Hybrid-Discrete} shows the most variance
among the four models, with two notable spikes above the \(+0.01\)
threshold at epoch~40 and epoch~53, both of which recover quickly
without leaving a persistent upward trend. \textbf{BERT-Hybrid-Fixed}
stays almost entirely within the healthy band across all 85 epochs,
with only two brief excursions below \(-0.01\) in the later phase of
training, confirming that the fixed fusion weight produces the most
stable generalization among the hybrid variants. \textbf{TCN-BERT4Rec}
displays moderate variance throughout training with several excursions
above \(+0.01\) concentrated in the final epochs, though the curve
remains bounded with no sign of a persistent gap opening up between
validation and test. \textbf{TGT-BERT4Rec} shows the tightest
oscillation of all four models, staying within the green band for
nearly all of its 55 training epochs with only a single dip below
\(-0.01\) near epoch~17, which is consistent with its strong and stable
validation performance. Taken together, all four models on ML-1M
demonstrate healthy generalization, with delta values remaining
small and bounded throughout training and no architecture showing
a systematic divergence between validation and test performance.

\clearpage
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{picture/figures/100k/delta_ndcg_overfitting_check_100k.png}
    \caption{\(\Delta\)NDCG@10 (Val \(-\) Test) per epoch on ML-100K for all four hybrid models. The green band marks the healthy range (\(\pm 0.01\)). Red dots indicate epochs where the gap exceeds the threshold.}
    \label{fig:delta_100k}
\end{figure}

On ML-100K (Figure~\ref{fig:delta_100k}), the delta curves are overall
tighter than on ML-1M, with all four models staying well within the
healthy band across the full training run. \textbf{BERT-Hybrid-Fixed}
shows the smoothest curve with virtually no excursions outside the
threshold, confirming that the fixed fusion weight does not introduce
instability even on a smaller dataset. \textbf{BERT-Hybrid-Discrete}
has a single spike near epoch~47 but recovers immediately.
\textbf{TCN-BERT4Rec} and \textbf{TGT-BERT4Rec} both train for
considerably more epochs on this dataset - TCN-BERT4Rec running to
epoch~90 and TGT-BERT4Rec to epoch~150 - yet both maintain stable
generalization throughout, with delta values fluctuating close to zero
and no persistent drift in either direction. The fact that TGT-BERT4Rec
remains well-behaved across 150 epochs is particularly notable, as
longer training runs typically increase the risk of overfitting; the
bounded delta curve confirms this is not the case here. Taken together,
the delta curves on both datasets confirm that none of the proposed
hybrid models overfit: the gap between validation and test performance
is small, bounded, and shows no systematic upward trend for any
architecture across either dataset.
...
Third, all hybrid models generalize reliably, with val--test delta
curves that remain within the healthy range throughout training,
showing that the additional graph branch does not cause overfitting.
BERT-Hybrid-Fixed achieves the strongest overall results on ML-1M,
...
\begin{table}[h]
\centering
\caption{Model efficiency and statistical significance on ML-1M. \(\bullet\) = proposed hybrid. * \(p < 0.05\).}
...
%  NOTE: Only ONE copy of this chapter is present here.
%        The original draft accidentally duplicated it verbatim.
% ============================================================
...
the seven description strategies; and Section~\ref{sec:ch4conclusion} concludes
the chapter. The experimental protocol is detailed in
Chapter~5 alongside the results.
...
initialize the item embedding table of a sequential recommendation model-either
SASRec \citep{kang2018sasrec} or BERT4Rec \citep{sun2019bert4rec}-replacing
the standard randomly-initialized or one-hot-based embedding layer.
...
\midrule
\textbf{T1 - Title Only} & The item name alone is sufficient &
...
\texttt{LLM2Sequential} pipeline. Each strategy encodes a distinct hypothesis
about what information the embedding model should receive, ranging from minimal
title-only encoding to rich hybrid descriptions combining multiple information
sources. The next chapter presents the experimental protocol and results,
analyzing the impact of each strategy on downstream recommendation performance.
...
\begin{table}[h]
\centering
\caption{Statistics of the Amazon Beauty dataset after 5-core filtering.
...
the user-centric portion (``For fans of: \ldots'') contributes tags that overlap
with the category and brand fields already present in the structured portion.
The marginal benefit of the additional framing may be insufficient to improve
over T2 alone.
...
narrative framing that slightly dilute the discriminative signal. Unlike
structured attributes, which present information in a consistent, predictable
format, prose descriptions vary in length, style, and informativeness across
items, making the resulting embedding space less uniform. However, the
degradation is marginal rather than substantial, suggesting that rich prose
encodings are not inherently harmful-they simply provide no clear benefit
over the baseline.
...
\(+4.8\%\) in NDCG@10. While these gains are smaller than those observed
...
fact that six of seven prompt strategies outperform the title-only
baseline confirms that description design matters, and the fact that
the magnitude of the effect is limited to a few percentage points is
...
demonstrates that simply adding text does not guarantee improvement:
description \emph{format} matters more than description \emph{length},
making strategy selection a genuine design choice rather than an
...
performance-T2, T4, T5, and T7 all outperform it. This indicates that average
...
interaction data. By contrast, embeddings produced by structured prompts, while
slightly less uniformly spread, may encode similarity relationships that are
more consistent with co-consumption patterns.
...
\section{Comparison with the Published \texttt{LLM2Sequential} Baseline}
...
in NDCG@10 and \(+2.54\%\) in HR@10. T7 - Structured-Comparative delivers the
largest absolute gain in HR@10 (\(+4.93\%\)), while T2 achieves the best MRR
(\(+6.79\%\) over T1).

\begin{table}[h]
\centering
\caption{Comparison of our best strategies against the published
\texttt{LLM2Sequential} baseline \citep{boz2025} on Amazon Beauty.
\(\Delta\) values are relative to the \citet{boz2025} title-only result (T1).}
\label{tab:vs_boz}
...
description passed to the encoder. The gains are modest but consistent, which
is expected given that the description strategy affects only the first stage of
the pipeline and that the BGE-M3 encoder is robust to moderate input
variations. This finding positions description strategy selection as a
practical, low-cost lever for improving LLM-based sequential recommendation
...
approach. The magnitude of the gain (\(+4.76\%\) in NDCG@10) is approximately
double that of most alternative strategies, underscoring the importance of
well-structured, concise metadata over free-form text.
...
relative to those items, creating a more informative neighborhood structure
for SASRec's attention mechanism. However, these strategies do not surpass
the simpler T2, indicating that the structured presentation of an item's own
attributes is more impactful than relational cross-item signals.

% FIX: adapted to reflect new results showing T3 is flat, not degrading
...
Natural language descriptions introduce variability in length, style, and
information density across items, which may offset any informational benefit.
...
embedding space where items are not compared on a uniform basis. The
negligible gap suggests that rich prose neither helps nor significantly
hurts-it is simply a neutral design choice.
...
strategy (T5) as a viable alternative when HR@20 is the priority
metric.
...
downstream recommendation accuracy, with the best strategy (T2 - Structured
Attributes) improving NDCG@10 by \(+4.76\%\) over the title-only baseline of
\citet{boz2025}. The Rich Prose strategy (T3) performs at parity with the
baseline (\(-0.61\%\)), demonstrating that simply adding more text does not
guarantee improvement and that description \emph{format} matters more than
description \emph{length}.
...
collaborative patterns rather than on geometric spread alone. The direct
comparison with \citet{boz2025} confirms that description strategy selection
is a practical, low-cost improvement lever that requires no architectural
changes. Together, these findings establish a first evidence-based guideline
for selecting item description strategies in LLM-based sequential
recommendation and open several directions for future work, including the
extension to additional datasets (Steam, MovieLens) and the exploration of
dataset-adaptive prompt strategies that automatically select the best
description format based on available metadata.


% ============================================================
%  GENERAL CONCLUSION (FRENCH) - kept as required by institution
%  Note: written in French per university requirements. All
%  other chapters are in English.
% ============================================================
...
\begin{figure}[htbp]
    ...
    \caption{Certificate of participation for the paper "Dynamic Sequence Modeling via Hybrid GNN-Transformer Architecture for Robust Sequential Recommendation" presented at the 2nd National Conference on Artificial Intelligence and Digital Transition (CAIDT'26), University of Kasdi Merbah, Ouargla, Algeria, April 29–30, 2026.}
...
delivering the best MRR. The Comparative (T5) and User-Centric (T4)
strategies also provided meaningful gains. Critically, the Rich Prose
strategy (T3) performed at parity with the baseline (\(-0.61\%\)), establishing
that providing more text does not necessarily yield better embeddings and that
the \emph{format} of item descriptions is as important as their \emph{content}.
These findings provide the first evidence-based guideline for selecting item
description strategies in LLM-based recommendation systems.
