...
\mbox{HybridFixed}    & Hybrid ablation & Fixed global fusion weight for all users \\
...
short-history users at \(0.1213\), followed by HybridDiscrete at
\(0.1132\) and \mbox{HybridFixed} at \(0.1102\), both outperforming the
...
\textbf{\mbox{HybridFixed}} leads at \(0.0963\), a substantial gain over
...
follows the same ranking: \mbox{HybridFixed} (\(0.0727\)) \(>\) TGT-BERT4Rec
(\(0.0689\)) \(>\) HybridDiscrete (\(0.0682\)) \(>\) BERT4Rec (\(0.0652\)).
...
Third, \mbox{HybridFixed} achieves the strongest overall results on ML-1M,
...
encoder, representing the Transformer-only baseline; (2) \mbox{HybridFixed}, which
adds a GNN encoder with a shared learnable fusion weight \(\lambda(u)\) for all
users; and (3) HybridDiscrete, which adds a GNN encoder with a user-specific
length-adaptive fusion weight \(\lambda(u)\). The results are reported in
...
+ GNN, fixed \(\lambda(u)\) (\mbox{HybridFixed})           & \textbf{0.1485} & \textbf{0.0727} & \textbf{0.0499} \\
+ GNN, adaptive \(\lambda(u)\) (HybridDiscrete)  & 0.1427 & 0.0682 & 0.0459 \\
\bottomrule
\end{tabular}
\end{table}
