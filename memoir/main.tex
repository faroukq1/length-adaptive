...
\begin{center}
  {\small\bfseries
    \begin{otherlanguage}{arabic}
      \arabicfont
      الجمهورية الجزائرية الديمقراطية الشعبية\\[0.5pt]
...
% ── University row ──────────────────────────────────────
\begin{center}
\begin{tabular}[c]{p{0.34\linewidth} c p{0.34\linewidth}}
  \raggedright
  {\footnotesize Badji Mokhtar -- Annaba University} &
...
\begin{flushleft}
  ...
  {\footnotesize \textbf{Sector:} Computer Science}\\[0.5pt]
  {\footnotesize \textbf{Specialty:} IT Systems and Decision}
\end{flushleft}
...
% ── thesis label ────────────────────────────────────────
\begin{center}
  {\large\bfseries Master’s Thesis}\\[1pt]
  {\normalsize \textbf{Submitted in partial fulfillment of the requirements for the Master's Degree}}
\end{center}
...
\begin{center}
  \begin{tabular}{lll}
    \textbf{Supervisor by:} & Samira Lagrini \quad & MCA \quad Badji Mokhtar -- Annaba University
  \end{tabular}
\end{center}
...
% ============================================================
%  ACADEMIC YEAR PAGE (top) + WATERMARK (center) - SEPARATE PAGE
% ============================================================
\newpage
\thispagestyle{empty}

% ── Watermark FIRST (so it renders behind text) ─────────────
\begin{tikzpicture}[remember picture, overlay]
  \node[opacity=0.07] at (current page.center)
    {\includegraphics[width=0.68\paperwidth]{picture/logo/logo.png}};
\end{tikzpicture}

% ── University Year at top ───────────────────────────────────
\begin{center}
  \vspace{2cm}
...
\begin{center}
  \itshape\large
  First and foremost, all praise and thanks are due to \textbf{Allah}, the Most Gracious, the Most Merciful, for granting me the strength, patience, and guidance to complete this work. Alhamdulillah.\\[12pt]

  To my beloved parents,\\[6pt]
  for every sacrifice they made and for their unwavering support throughout my journey.\\[12pt]

  To my supervisor, Madame Samira Lagrini,\\[6pt]
  for your thoughtful guidance, your wise advice, and your trust in me. Thanks to you, I have grown both intellectually and personally. Your support has been invaluable.\\[12pt]

  To my family and friends,\\[6pt]
  for their constant encouragement and support.\\[12pt]

  To my dear friends outside the university,\\[6pt]
  for your sincere friendship, your laughter, and your unwavering presence in my life. You are a true blessing.\\[12pt]

  To the entire SID cohort (2024--2026),\\[6pt]
  for the unforgettable journey, the shared efforts, and the wonderful memories. You made this experience truly unique.\\[18pt]
...
\textbf{My sisters Sara and Amina, and my brother Fateh} -- your kindness, your laughter, your strength, and your unwavering support have been a true gift. Thank you for always being there for me and for always having my back.
...
You were incredibly patient with me throughout this journey, always taking the
time to listen, explain, and guide me whenever I faced difficulties.
...
\textbf{Oussama Safwan Souici}, \textbf{Aymen Kennouche}, \textbf{Mezouari Abdelkader}, \textbf{Deneche Seif El Islam},
\textbf{Nezzar Nour El Houda}, \textbf{Necib Chahd} and \textbf{Saadi Nada} --
Thank you for every moment of support, for every helping hand, and for always being by my side when I needed you most. Your presence and encouragement have meant everything to me.
...
Recommendation systems have become an essential part of our digital experience, helping
users discover relevant content across platforms ranging from streaming services to e-
commerce. Yet despite significant progress, two important challenges remain: how to
intelligently combine different sources of information about user behavior, and how to best
describe items to modern AI language models so that they generate more meaningful
representations. This thesis addresses both challenges through two original contributions.

The first contribution proposes a recommendation model that adapts itself to each user based
on how much interaction history they have accumulated. The core idea is straightforward: a
user who has rated only a handful of items should be treated differently from one who has
been actively engaging with a platform for years. By automatically adjusting the balance
between general popularity signals and personal behavioral patterns, the proposed model
provides more relevant recommendations for all types of users. Experiments conducted on
two well-known movie rating datasets confirm that this adaptive strategy consistently
outperforms existing approaches, with notable improvements across all evaluation metrics.

The second contribution investigates a question that is often overlooked in the
recommendation literature: when we ask an AI language model to understand an item, does
the way we phrase its description actually matter? To answer this, seven different ways of
describing items were designed and tested, ranging from a simple title to richly detailed and
user-oriented descriptions. The results are both clear and surprising: a well-structured and
concise description consistently outperforms longer, more elaborate ones. This finding
suggests that clarity and structure are more valuable than sheer descriptive richness when
working with language models.

Taken together, these two contributions offer a more flexible and informed approach to
sequential recommendation, one that better accounts for user diversity and the untapped
potential of how we communicate item information to AI systems.

\vspace{0.5cm}
\textbf{Keywords:} Sequential Recommendation, GNN-Transformer, Adaptive Fusion, Length-Aware Modeling, Large Language Models, Item Description Strategies, Prompt Engineering, BGE-M3, LLM2Sequential, SASRec, BERT4Rec, LightGCN.

\newpage
\section*{Résumé}
\vspace{0.5cm}

Les systèmes de recommandation sont devenus une composante essentielle de notre
expérience numérique, aidant les utilisateurs à découvrir des contenus pertinents sur des
plateformes allant des services de streaming au commerce en ligne. Pourtant, malgré des
avancées significatives, deux défis importants demeurent : comment combiner intelligemment
différentes sources d'information sur le comportement des utilisateurs, et comment décrire au
mieux les éléments aux modèles de langage modernes afin qu'ils génèrent des représentations
plus pertinentes. Cette thèse aborde ces deux défis à travers deux contributions originales.

La première contribution propose un modèle de recommandation qui s'adapte à chaque
utilisateur en fonction de la quantité d'historique d'interactions qu'il a accumulée. L'idée
centrale est simple : un utilisateur qui n'a évalué qu'une poignée d'éléments doit être traité
différemment de celui qui interagit activement avec une plateforme depuis des années. En
ajustant automatiquement l'équilibre entre les signaux de popularité générale et les
comportements personnels, le modèle proposé fournit des recommandations plus pertinentes
pour tous les types d'utilisateurs. Des expériences menées sur deux jeux de données bien
connus d'évaluation de films confirment que cette stratégie adaptative surpasse
systématiquement les approches existantes, avec des améliorations notables sur l'ensemble
des métriques d'évaluation.

La deuxième contribution examine une question souvent négligée dans la littérature sur la
recommandation : lorsque nous demandons à un modèle de langage de comprendre un
élément, la façon dont nous formulons sa description a-t-elle vraiment de l'importance ? Pour
répondre à cette question, sept manières différentes de décrire les éléments ont été conçues et
testées, allant d'un simple titre à des descriptions riches et orientées utilisateur. Les résultats
sont à la fois clairs et surprenants : une description bien structurée et concise surpasse
systématiquement les descriptions plus longues et plus élaborées. Ce constat suggère que la
clarté et la structure ont plus de valeur que la richesse descriptive lorsqu'on travaille avec
des modèles de langage.

Ces deux contributions offrent une approche plus flexible et mieux informée de la
recommandation séquentielle, tenant mieux compte de la diversité des utilisateurs et du
potentiel inexploité de la façon dont nous communiquons les informations sur les éléments
aux systèmes d'intelligence artificielle.

\vspace{0.5cm}
\textbf{Mots-clés :} Recommandation séquentielle, GNN-Transformer, fusion adaptative, modélisation dépendante de la longueur, grands modèles de langage, stratégies de description d'items, ingénierie des prompts, BGE-M3, LLM2Sequential, SASRec, BERT4Rec, LightGCN.
...
their interests, and can impair or paralyze the decision-making process.
...
Among early contributions, \citep{hidasi2016gru4rec} showed that gated
...
data alone cannot provide \citep{harte2023leveraging,liu2024llmers}. However,
...
This thesis addresses two complementary research problems:

\medskip
\noindent\textbf{Research Problem 1} - \textit{How to dynamically adapt the
fusion between global graph signals and local sequential modelling according
to each user's interaction history profile?}

\medskip
\noindent\textbf{Research Problem 2} - \textit{How does the choice of item
...
interest profiles so as to recommend relevant content by analyzing
...
concentrates only on this type of system. Formally they are defined below.
...
\begin{figure}[htbp]
    ...
    % CORRECTION: shortened caption to one explicit line
    \caption{Architecture of BERT4Rec: a bidirectional Transformer-based sequential model.}
...
\citep{harte2023leveraging,liu2024llmers,wu2023survey}.
...
profiles \citep{harte2023leveraging,liu2024llmers,wu2023survey}. Survey work also
...
thesis, where different item description strategies are analyzed in the context
...
\justifying
To evaluate the performance of sequential recommendation models, this thesis
...
\begin{table}[h]
\centering
% Adjusted column widths to give more space to the formula column
\begin{tabular}{p{2.2cm} p{5.8cm} p{5.0cm}}
\toprule
...
Together, these two gaps define the positioning of this thesis: the first
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{picture/figures/schemas/sparse_dense_comparison.png}
...
\justifying
The simplest variant applies a single fixed blending weight \(\lambda(u)\) to all
...
bidirectional self-attention. The value of \(\lambda(u)\) is set manually before
...
\justifying
\textbf{Fixed Fusion} applies a single, global scalar weight \(\lambda(u)\) to all
...
(\(\lambda_{\text{short}}, \lambda_{\text{mid}}, \lambda_{\text{long}}\)), which is
...
TGAT (Xu et al., 2020), adapted here as a sequence encoder with a dual
attention pathway and a learnable cosine time encoding
$\cos(\mathbf{W} \cdot t)$. Recent works such as Temporal Graph Networks (TGN) (Rossi et al., 2020) have also addressed temporal dynamics in graphs. Concretely, it augments the attention keys with learned time encodings
...
pan2024survey}. Four concrete variants were derived from this framework: \mbox{HybridFixed}, HybridDiscrete, TGT-BERT4Rec, and TCN-BERT4Rec.
...
\(\lambda_{\text{short}}\), \(\lambda_{\text{mid}}\), and \(\lambda_{\text{long}}\)
...
\(\lambda_{\text{short}}\)       & 0.3     & Tuned on validation set \\
\(\lambda_{\text{mid}}\)         & 0.5     & Tuned on validation set \\
\(\lambda_{\text{long}}\)        & 0.7     & Tuned on validation set \\
\midrule
...
\textbf{\mbox{HybridFixed}} uses a single global fusion weight \(\lambda(u)\) shared
...
\mbox{HybridFixed} (Fixed Fusion)    & Hybrid ablation & Fixed global fusion weight for all users \\
HybridDiscrete (Discrete Fusion) & Hybrid ablation & Length-dependent fusion bins for user groups \\
...
\textbf{Model} &
HR@5 & HR@10 & HR@20 & NDCG@5 & NDCG@10 & NDCG@20 & MRR@5 & MRR@10 & MRR@20 \\
\midrule
...
\mbox{HybridFixed}\(^\bullet\)    & \textbf{0.0852} & \textbf{0.1485} & \textbf{0.2511} & \textbf{0.0524} & \textbf{0.0727} & \textbf{0.0984} & \textbf{0.0417} & \textbf{0.0499} & \textbf{0.0569} \\
\bottomrule
\end{tabular}}
\end{table}
...
\mbox{HybridFixed}\(^\bullet\)    & \textbf{0.0501} & \underline{0.0864} & \underline{0.1727} & \underline{0.0279} & 0.0393 & \underline{0.0610} & \underline{0.0207} & 0.0252 & \underline{0.0311} \\
\bottomrule
\end{tabular}}
\end{table}
...
\textbf{\mbox{HybridFixed}} achieves the strongest overall performance
...
the recommendation list is extended. \textbf{HybridDiscrete}
...
The two GNN-augmented hybrid variants, \mbox{HybridFixed} and
HybridDiscrete, show a smaller advantage over BERT4Rec compared
...
% Force all previous figures to appear before Summary
...
\begin{figure}[H]
    ...
    {picture/figures/1m/delta_ndcg_overfitting_check_1m.png}
    \caption{Delta NDCG@10 (Validation−Test) on ML-1M green band healthy range ±0.01 outside potential overfitting.}
    \label{fig:delta_ndcg_1m}
\end{figure}

\justifying
Figure~\ref{fig:delta_ndcg_1m} illustrates the progression of
$\Delta$NDCG@10 (Validation NDCG@10 minus Test NDCG@10) throughout
training for the four hybrid recommendation models on the ML-1M dataset.
The green shaded area corresponds to the acceptable generalization
interval ($\pm 0.01$), where validation and test scores remain closely
matched. Values outside this interval would indicate potential
overfitting or under-generalization.

All four models exhibit notably smooth and stable curves.
HybridDiscrete fluctuates mildly around zero and remains entirely within
the healthy range. HybridFixed also stays within the threshold, showing
small positive and negative variations without any significant divergence
between validation and test performance. TCN-BERT4Rec maintains a stable
pattern centered near zero, with only minor oscillations and no overfitting
indicators. TGT-BERT4Rec displays a gradual upward trend in later epochs,
reflecting slightly higher validation scores than test scores, yet all
values remain comfortably within the $\pm 0.01$ boundary.

Overall, the ML-1M results provide strong evidence of good
generalization for all models. The absence of threshold violations and
the consistently small validation--test gaps indicate that the larger
dataset helps reduce variance and improves training stability, resulting
in more reliable performance on unseen data.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth,height=0.7\textheight,keepaspectratio]
    {picture/figures/100k/delta_ndcg_overfitting_check_100k.png}
    \caption{Delta NDCG@10 (Validation−Test) on ML-100K green band healthy range ±0.01 outside potential overfitting.}
    \label{fig:delta_ndcg_100k}
\end{figure}

\justifying
Figure~\ref{fig:delta_ndcg_100k} presents the evolution of
$\Delta$NDCG@10 (Validation NDCG@10 minus Test NDCG@10) for the four
recommendation models trained on the ML-100K dataset. The shaded green
region ($\pm 0.01$) represents the healthy generalization range, where
validation and test performance remain closely aligned. Values above
$+0.01$ indicate potential overfitting; values below $-0.01$ do not
indicate overfitting (validation simply underperforms test).

Overall, the models exhibit relatively stable behavior, with
$\Delta$NDCG@10 values oscillating around zero for most epochs.
HybridDiscrete remains largely within the healthy range, with only a few
isolated positive spikes slightly above $+0.01$, suggesting mild
overfitting episodes. TCN-BERT4Rec presents multiple positive peaks above
$+0.01$, indicating clearer periods where validation performance exceeds
test performance – a sign of potential overfitting, though these
deviations are short-lived. HybridFixed shows several negative excursions
below $-0.01$ near the end of training; this is not overfitting but rather
occasional instability where validation lags behind test performance.
TGT-BERT4Rec demonstrates the most stable behavior among the four models,
with only one significant negative deviation and otherwise remaining close
to zero.

The analysis suggests that TCN-BERT4Rec and, to a lesser extent,
HybridDiscrete show isolated signs of overfitting on ML-100K, while
HybridFixed's negative excursions are not a generalization concern.
Overall, the relatively small validation--test gaps indicate that
performance estimates obtained on the validation set remain reasonably
representative of unseen test data.

\subsection{Summary}

\justifying
Across both datasets and all evaluation dimensions, the results
consistently support three conclusions. First, on ML-1M, all hybrid
variants outperform BERT4Rec. On ML-100K, TCN-BERT4Rec and
TGT-BERT4Rec achieve the best performance, confirming that the
optimal architecture depends on dataset characteristics. Second, the
advantage is largest for short-history users, directly confirming the
length-adaptive motivation: when sequential evidence is sparse, the
GNN collaborative signal provides a meaningful complementary source
of information. Third, \mbox{HybridFixed} achieves the strongest
overall results on ML-1M, while TCN-BERT4Rec leads on ML-100K,
suggesting that the most effective architecture may depend on dataset
scale and sequence length distribution.

\subsection*{Effect of the GNN Encoder}

\justifying
Adding the GNN encoder with a fixed fusion weight (\mbox{HybridFixed}) improves
...
\justifying
HybridDiscrete achieves intermediate overall metrics compared to \mbox{HybridFixed},
...
three factors. First, the learnable scalar \(\lambda(u)\) in \mbox{HybridFixed} is
...
receive sharply different fusion weights (\(\lambda_{\text{short}} = 0.3\) vs
\(\lambda_{\text{mid}} = 0.5\)) despite having nearly identical sequential
evidence. Third, because the fusion is applied at the item embedding table
level before the Transformer blocks, \mbox{HybridFixed} sees a single globally
...
\mbox{HybridFixed}, HybridDiscrete, and TGT-BERT4Rec achieve statistically
...
\mbox{HybridFixed}\(^\bullet\)    & 1.8M & 88  & 0.1485 & \(+8.1\%\)*  \\
\bottomrule
\end{tabular}
\end{table}
...
thesis by investigating how item textual description strategies influence the
...
This chapter presents the second contribution of this thesis: a systematic
...
\citep{boz2025}, we design and evaluate seven distinct prompt strategies that
...
introduced by \citep{boz2025}.
The approach adopted in this thesis bridges the gap between textual item
semantics and sequential recommendation models through a three-stage pipeline,
building on the framework of \citep{boz2025}. Figure~\ref{fig:llm_seq_pipeline}
...
\begin{tabular}{p{2.2cm} p{3.5cm} p{7.0cm}}
\toprule
...
published \texttt{LLM2Sequential} baseline of \citep{boz2025}.
...
Contribution~1 of this thesis and consistent with standard practice in
...
the findings reported in \citep{boz2025} and validating that the
...
\citep{boz2025} on the same Amazon Beauty dataset using the same SASRec
backbone. The \citep{boz2025} pipeline uses the item title as the sole textual
...
baseline of \citep{boz2025}. The Rich Prose strategy (T3) performs at parity
...
The direct comparison with \citep{boz2025} confirms that description strategy
...
\justifying
This thesis addressed two open problems in sequential recommendation systems:
...
in two variants: \mbox{HybridFixed} with a learnable global fusion weight, and
...
graph-based baselines. \mbox{HybridFixed} achieved improvements of \(+8.1\%\) in
HR@10, \(+11.5\%\) in NDCG@10, and \(+14.2\%\) in MRR over the BERT4Rec
baseline on ML-1M. On ML-100K, TCN-BERT4Rec achieved the best performance,
confirming that the most effective architecture depends on dataset scale and
sequence length distribution. The per-bucket analysis confirmed that the
adaptive fusion mechanism provides targeted gains for short-history users, who
are systematically underserved by Transformer-only models. The ablation study
...
\textbf{Bridging the Two Contributions.}
Although presented separately, the two contributions are complementary.
The adaptive fusion mechanism (Contribution 1) dynamically balances global
graph signals and local sequential patterns based on user history length.
Meanwhile, the PromptCraft strategies (Contribution 2) improve the semantic
richness of item representations, which could further enhance the GNN
collaborative signal for sparse users. Exploring how semantically enriched
embeddings interact with user-adaptive fusion remains a promising direction
for future research.
...
balance between graph and sequence signals. For Contribution 2, the experiments
were conducted on a single dataset (Amazon Beauty) and a single recommendation
model (SASRec). The generalizability of the findings to other domains with
different metadata profiles remains an open question.

\textbf{Future Directions.}
First, extending the adaptive fusion mechanism to a fully learnable, continuous
function could further improve performance across heterogeneous user populations.
Second, evaluating the PromptCraft strategies on additional datasets (Steam,
MovieLens, Delivery Hero) would establish the extent to which findings generalize
across domains. Third, exploring dataset-adaptive prompt selection could make the
PromptCraft framework practical for real-world deployment without manual
strategy tuning. Fourth, investigating the interaction between prompt strategy
and the choice of embedding model could reveal whether certain strategies benefit
more from larger or differently-trained embedding models. Fifth, extending the
PromptCraft methodology to other recommendation paradigms beyond sequential
recommendation would broaden the impact of the finding that item description
design is a first-class choice in LLM-augmented recommendation systems.

\textbf{Final Take-Home Message.}
This thesis demonstrates that personalizing hybrid architectures based on user
history length and carefully designing item textual descriptions both yield
significant, practical improvements in sequential recommendation. The proposed
models are not only empirically effective but also conceptually simple, making
them suitable for real-world deployment. We hope these contributions inspire
further research toward more adaptive and semantically aware recommendation
systems.
...
Ouled Meriem, Farouk, Samira Lagrini, Sarra Benzaid, and Nada Zeraoui.
...
\end{document}}ent}}document}}ent}}{References}]

\end{document}}ent}}document}}ent}}